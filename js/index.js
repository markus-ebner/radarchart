...
playerHexagon.setAttribute('style', 'stroke:hsla(80, 70%, 70%, 1); fill:hsla(80, 70%, 70%, .5); marker-start: url(#markerCircle); marker-mid: url(#markerCircle);');
...
var ronaldo = new Player({
  infos: {
    photo: 'http://www.sevillafc.es/sites/default/files/header/image/escudosfc.png',
...
var silva = new Player({
  infos: {
    photo: 'https://upload.wikimedia.org/wikipedia/en/thumb/7/7a/Manchester_United_FC_crest.svg/1010px-Manchester_United_FC_crest.svg.png',
    name: 'Thiago Silva',
...
var pogba = new Player({
  infos: {
    photo: 'https://upload.wikimedia.org/wikipedia/de/thumb/f/f1/Olympique_Lyon.svg/1200px-Olympique_Lyon.svg.png',
    name: 'Paul Pogba',
    team: 'Juventus FC',
    age: 22,
    nation: 'France',
...
// Pogba Creation
var dortmund = new Player({
    infos: {
        photo: 'http://bilder.bild.de/fotos/borussia-dortmund-47475354/Bild/2.bild.svg',
...
// Pogba Creation
var turin = new Player({
    infos: {
        photo: 'https://upload.wikimedia.org/wikipedia/de/2/2e/Torino_FC_Logo.svg',
        name: 'FC Turin',
        team: 'Juventus FC',
        age: 22,
        nation: 'France',
...
function displayName(event) {
  playerLineArray.forEach(function(line) {
    ...
    } else if (event.target.classList.contains('dortmund')) {
        if (line.classList.contains('dortmund')) {
            line.classList.toggle('playerLineShow');
        }
    } else if (event.target.classList.contains('turin')) {
        if (line.classList.contains('turin')) {
            line.classList.toggle('playerLineShow');
        }
    }
  });
  ...
  otherPlayerArray.forEach(function(el) {
    if (el.classList.contains('playerName')) {
      ...
      if (event.target.classList.contains('dortmund') && el.classList.contains('dortmund')){
          event.type == 'mouseleave' ? el.classList.add('transparent') : el.classList.remove('transparent');
      }
      if (event.target.classList.contains('turin') && el.classList.contains('turin')){
          event.type == 'mouseleave' ? el.classList.add('transparent') : el.classList.remove('transparent');
      }
    }
  });
...
function runStats(event) {
  if (event.target.classList.contains('silva')) {
    ...
    removeFilter('imagePogba');
      ...
      newPlayerAllStats = dortmund.averageStats;
      removeFilter('imageDortmund');
  } else if (event.target.classList.contains('turin')) {
      newPlayerAllStats = turin.averageStats;
      removeFilter('imageTurin');
  }
}
...
getPlayerStats(dortmund);
getPlayerStats(turin);
anim();